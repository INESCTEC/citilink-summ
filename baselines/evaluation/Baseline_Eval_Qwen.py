...
# Mantemos o modelo multilingual otimizado para português
...
def parse_args() -> argparse.Namespace:
    ...
    """
    parser = argparse.ArgumentParser(description="Evaluate Qwen generated summaries from a JSONL file.")
    parser.add_argument(
        "--input_file",
        type=str,
        required=True,
        help="Path to the JSONL file containing the generated summaries and references (output of the Qwen generation script)."
    )
    parser.add_argument(
        "--output_file",
        type=str,
        # Nome de arquivo de saída ajustado para Qwen
        default="qwen_evaluation_results.json",
        help="File path to save the final Qwen evaluation results (JSON format)."
    )
...
def load_generated_data(file_path: str) -> Tuple[List[str], List[str]]:
    ...
    with open(file_path, "r", encoding="utf-8") as f:
        # Abertura do ficheiro com barra de progresso (tqdm)
